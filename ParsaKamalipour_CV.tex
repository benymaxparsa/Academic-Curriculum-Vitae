...
%	\subsection{Journal papers}
	\begin{itemize}
		...
		\item \textit{\href{https://benymaxparsa.github.io/publication/p6}{Spider Community Detection: Seeded Geodesic Expansion with Modularity-Guided Refinement and Greedy Merge Matching \ExternalLink}}
			\begin{itemize}
				\item H. Harutyunyan, \textbf{Parsa Kamalipour} — \textit{Computers, Special Issue: Recent Advances in Social Networks and Social Media (\textbf{accepted}), 2026, \href{https://doi.org/10.3390/computers1010000}{\textbf{doi}.\ExternalLink}}				\end{itemize}
		...
		\item \textit{\href{https://benymaxparsa.github.io/publication/p5}{From Dense Graphs to Meaningful Communities: Assessing Community Quality Using Geodesic Distance Modularity on Metric Backbone-Sparsified Networks \ExternalLink}}
			\begin{itemize}
				\item \textbf{Parsa Kamalipour}, H. Harutyunyan — \textit{SNAMS 2025 (\textbf{accepted})}
				\end{itemize}
		...
		\item \textit{\href{https://benymaxparsa.github.io/publication/p4}{LLM-Based Code Translation for Cross-Language Refactoring Mining \ExternalLink}}
			\begin{itemize}
				\item I. Hemati Moghadam, M. M. Afkhami, V. Zaytsev, M. H. Ashoori, H. Bazmandegan, and \textbf{Parsa Kamalipour}  — \textit{Empirical Software Engineering (\textbf{in revision})}
				\end{itemize}
...
\cventry{Aug 2024 -- Present}
  ...
  {\begin{itemize}
\item Designed the \textbf{Spider graph} community detection algorithm combining
\textbf{geodesic expansion, modularity-guided refinement, and greedy merge matching}.
\item Benchmarked Spider on \textbf{14} real-world networks
(up to \textbf{8{,}035 nodes / 183{,}663 edges}) against \textbf{Leiden, Louvain, and Infomap},
achieving \textbf{8--15\%} improvement in \textbf{NMI, modularity, and F1-score}.
\item Applied \textbf{metric backbone}, yielding on average \textbf{65\% edge reduction},
and introduced \textbf{Weighted Average Geodesic Distance Modularity (wGDM)} to normalize and balance the original GDM
for evaluating local community quality on sparsified graphs.
\item Built a reproducible experimental pipeline with fixed random seeds,
baseline implementations, and automated evaluation scripts.\end{itemize}}
...
\cventry{Aug 2023 -- Mar 2024}
  ...
  {\begin{itemize}%
\item Implemented the \textbf{KotlinCode2Text} parser and integrated it into the \textbf{RefDetect} framework for automated refactoring detection.
\item Built two refactoring datasets used for empirical evaluation in the \textit{SANER 2024} study.
\item Improved analysis reliability and runtime through targeted debugging and algorithmic refinements.
\item Explored \textbf{LLM-based prompt engineering} for cross-language code translation in refactoring mining.
  \end{itemize}}
...
\cventry{Aug 2021 -- Mar 2024}
  ...
  {\begin{itemize}%
\item Developed the \textbf{CRLG} community detection framework based on an randomized algorithm using local and global network information.
\item Implemented weighted probabilistic seeding and similarity-driven community assignment with heuristic community merging.
\item Evaluated on real networks and GN/LFR benchmarks, achieving up to \textbf{+10\%} improvement over
\textbf{LCDR, MOACO, Node2vec-SC, NE-N2V, CDASS, and TS} using \textbf{NMI, modularity, and density}.
\end{itemize}}
	%-----------------------------------------------------------------------------------------------------------
...
\cventry{Sep 2024 -- Present}
  ...
  {}
  {\begin{itemize}%
\item Delivered \textbf{tutorials} and \textbf{laboratory demonstrations}, \textbf{graded assignments and exams}, and provided student support through \textbf{Programmer On Duty (POD)} sessions, office hours, and detailed feedback on coursework and projects. Courses:  
%    \item 
      \begin{multicols}{2}
      \begin{itemize}
      	\vspace*{-6mm}
...
\cventry{Mar 2021 -- Jan 2024}
  ...
  {}
  {\begin{itemize}
\item Served as \textbf{Head TA and Tutorial Leader} for multiple foundational CS courses, mentoring students, coordinating grading, and collaborating with faculty to design assignments and support student projects in various courses.
    \item Courses: Data Structures, Algorithms Design, Discrete Mathematics, Operating Systems, Information Retrieval, Software Engineering, Database Systems, Artificial Intelligence, Data Mining.
  \end{itemize}}
...
%		\item Approximation \& Randomized Algorithms
		\item Machine Learning \& Graph Mining
		\item Social Networks Analysis \& Complex Networks
	\end{itemize}
\end{multicols}
...
\cvitem{2025}{\textbf{Awarded Concordia Conference and Exposition Allowance} -- Concordia University}
...
\cvitem{2023}{\textbf{Top Researcher Award} -- Earned this prestige award among all undergraduate students in Kerman Province}
...
{\href{https://github.com/Fall2024-MLProjects/CommunityDetection}{Evaluation of Scalable Training Strategies for Graph Neural Networks \ExternalLink}}
{Concordia University, Montréal}
{}
{
\begin{itemize}
\item Designed and implemented scalable GNN-based community detection pipelines using \textbf{GCN} and \textbf{GraphSAGE} architectures with full-batch training, neighbor sampling, and graph partitioning strategies.
\item Conducted extensive experiments on \textbf{SBM (1K, 10K)}, \textbf{CORA}, and \textbf{Reddit} datasets, demonstrating that neighbor sampling and graph partitioning enable training on large graphs where full-batch methods fail due to memory constraints.
\item Achieved up to \textbf{90\% accuracy} on SBM (10K nodes) with graph partitioning while reducing memory footprint, and enabled scalable training on Reddit where full-batch methods resulted in out-of-memory failures.
\item Analyzed trade-offs between accuracy, training time, and memory usage, providing practical guidelines for scalable GNN deployment in real-world large-scale social networks.
%\item Implemented models using PyTorch Geometric, including \texttt{GCNConv}, \texttt{GraphSAGEConv}, \texttt{NeighborLoader}, and \texttt{ClusterLoader}.
\end{itemize}
}
...
{\href{https://github.com/COMP6651-ADT-Project-Fall2024/minimum-cost-flow}{Experimental Study of Network Flow Optimization Algorithms \ExternalLink}}
{Concordia University, Montréal}
{}
{
\begin{itemize}
\item Implemented the \textbf{Successive Shortest Path} algorithm from scratch, including residual graph construction, Bellman–Ford based minimum-cost path extraction, and flow augmentation logic.
\item Designed and executed large-scale experimental evaluation on \textbf{randomized Euclidean directed graphs} across 28 configurations with varying density ($r$), capacity bounds, and cost regimes.
\item Compared \textbf{SSP, Capacity Scaling, Scaling-SSP, and Primal–Dual} algorithms using metrics including total cost, flow value, number of augmenting paths, mean path length, and proportional path length.
\item Demonstrated that the \textbf{Primal–Dual algorithm consistently achieves optimal minimum cost}, while SSP achieves competitive performance in sparse regimes and degrades in dense graphs.
%\item Built an automated correctness testing framework validating all implementations against manually computed ground-truth instances.
\end{itemize}
}
%
...
%}{}{}{
%	the Red-Black Tree, the AVL Tree, the Trie Dictionary, Threaded Binary Tree, the Sparse Matrix Linked List, the Rat in the maze problem
%}
...
%\cvdoubleitem{Graph Data \& Pipelines}{SNAP \& DBLP Datasets, Large-Scale Graph Processing, Experimental Reproducibility}
%              {Software Engineering}{Algorithm Prototyping, Debugging, Unit Testing, Git}
...
%
%\cvitem{Undergraduate}{Design and Analysis of Algorithms, Data Structures, Discrete Mathematics, Artificial Intelligence, Software Engineering, Fundamentals of Data Mining}
%%\cvdoubleitem{Programming Language Design}{18/20}{Database}{16/20}
%	\cvdoubleitem{Theory of Machines \& Languages}{17.9/20}{The principles of Compiler Design}{20/20}
%$\filledstar$ \textit{\href{https://benymaxparsa.github.io/selected_courses}{Click here to see more \ExternalLink}}
%
...
\textbf{Persian}: Native & \textbf{English}: Proficient (C1) & \textbf{French}: Pre-intermediate (A2) \\
\end{tabular*}


%\section{Volunteer Experience}
%
%\cventry{Nov 2022 -- Sep 2023}
